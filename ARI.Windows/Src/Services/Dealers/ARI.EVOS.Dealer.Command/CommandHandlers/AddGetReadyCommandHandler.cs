...
namespace ARI.EVOS.Dealer.Command.CommandHandlers
{
    ...
    public class AddGetReadyCommandHandler : ICommandHandler<AddGetReadyCommand, string>
    {
        ...
        public async Task<Result<string>> HandleAsync(AddGetReadyCommand command)
        {
            if (_getReadyValidation.IsValid(command))
            {
                ...
                if (isExist.HasValue)
                {
                    ...
                    {
                        return await ValidateGetReady(command);
                    }
                    else
                    {
        ...
        /// <returns></returns>
        private async Task<Result<string>> ValidateGetReady(AddGetReadyCommand commandAdd)
        {
