...
namespace ARI.EVOS.Dealer.Command.CommandHandlers
{
    ...
    public class DeleteGetReadyCommandHandler : ICommandHandler<DeleteGetReadyCommand, string>
    {
        ...
        public DeleteGetReadyCommandHandler(IDealerUnitOfWork dealerUnitOfWork)
        {
        ...
        public async Task<Result<string>> HandleAsync(DeleteGetReadyCommand command)
        {
            foreach (var grVehicle in command.GetReadyCategories)
            {
                ...
                {
                    var vehicle = vehicleDet.Result.Value.FirstOrDefault();
