...
{
    public class AddGetReadyCommandHandlerTest
    {
        ...
        public AddGetReadyCommandHandlerTest()
        {
    ...
    public class DealerDto : Domain.Models.Dealer.Aggregate.Dealer
    {
        public DealerDto()
        {
        ...
        public Domain.Models.Dealer.Aggregate.Dealer GetDealers()
        {
            var dealerId = DealerId.Create("test123");
            var countryCode = CountryCode.Create("CA");
            var makeCode = MakeCode.Create("CH",countryCode);
            var dealerDto = new DealerDto() { DealerId = dealerId, CountryCode = countryCode, MakeCode = makeCode };
            return dealerDto;
        }
    }
}
